...
def selecteddata():
    ...
    per_page = int(request.args.get('per_page', 100))  # Get items per page, default to 5
...
def updatemail():
    try:
        ...
        if 'user_id' in session:
            ...
            existing_statement = db.session.query(candidate_history).filter_by(candidate_id=id,
                        ...
                        for field, values in changes.items():
                            ...
                            if field == 'password':
                                ...
                                if values['from'] != values['to']:
                                    if isinstance(values['from'], int):
                                        old_value_str = str(values['from'])
                                    else:
...
def view(form_id, form_type):
    ...
    if form_type == 'New Deals Contract Signed' or form_type == 'Marketing':
        ...
        formdata = Otherfinal.query.filter(Otherfinal.id == form_id,Hrforms.org_id == org_id).first()
                ...
                margin-top: 10px;
                               ...
                               <body>
                                   <div class="container">
                                       <img class="cont-img" src="https://www.click-hr.com/static/img/clickhr.png">
                                         <div class="hello"></div>
                                       <p>Weekly Target is set filled by {user_name} </p>
...
def start_scheduler():
    ...
    schedule.every().day.at("19:15").do(arc_mails_auto)
    schedule.every().day.at("19:15").do(send_email)
    while True:
